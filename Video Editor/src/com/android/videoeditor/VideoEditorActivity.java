...
public class VideoEditorActivity extends VideoEditorBaseActivity implements SurfaceHolder.Callback {
	...
	private final TimelineRelativeLayout.LayoutCallback mLayoutCallback = new TimelineRelativeLayout.LayoutCallback() {
	...
	public void onCreate(Bundle savedInstanceState) {
			...
			public void onTrimMediaItem(MovieMediaItem mediaItem, long timeMs) {
				...
				if (mProject != null && isPreviewPlaying()) {
					if (mediaItem.isVideoClip()) {
						...
						mPreviewThread.previewFrame(mProject, mProject.getMediaItemBeginTime(mediaItem.getId()) + timeMs,
		...
		if (savedInstanceState != null) {
			mInsertMediaItemAfterMediaItemId = savedInstanceState.getString(STATE_INSERT_AFTER_MEDIA_ITEM_ID);
		...
		mSurfaceView.setGestureListener(new GestureDetector(this, new GestureDetector.SimpleOnGestureListener() {
			@Override
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(R.id.menu_item_import_audio).setVisible(haveProject && mProject.getAudioTracks().size() == 0 && haveMediaItems);
		menu.findItem(R.id.menu_item_change_aspect_ratio).setVisible(haveProject && mProject.hasMultipleAspectRatios());
		...
		menu.findItem(R.id.menu_item_play_exported_movie).setVisible(enableMenu && mProject.getExportedMovieUri() != null);
		menu.findItem(R.id.menu_item_share_movie).setVisible(enableMenu && mProject.getExportedMovieUri() != null);
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_item_capture_video: {
			...
			mCaptureMediaUri = getContentResolver().insert(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, values);
		...
		case R.id.menu_item_capture_image: {
			...
			// Create parameters for Intent with filename
			final ContentValues values = new ContentValues();
			mCaptureMediaUri = getContentResolver().insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values);
	...
	public Dialog onCreateDialog(int id, final Bundle bundle) {
		...
		case DIALOG_CHOOSE_ASPECT_RATIO_ID: {
			...
			final ArrayList<Integer> aspectRatios = bundle.getIntegerArrayList(PARAM_ASPECT_RATIOS_LIST);
			...
			builder.setSingleChoiceItems(aspectRatioStrings, bundle.getInt(PARAM_CURRENT_ASPECT_RATIO_INDEX), new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {
					final int aspectRatio = aspectRatios.get(which);
					ApiService.setAspectRatio(VideoEditorActivity.this, mProjectPath, aspectRatio);
		...
		case DIALOG_DELETE_PROJECT_ID: {
			return AlertDialogs.createAlert(this, getString(R.string.editor_delete_project), 0, getString(R.string.editor_delete_project_question),
		...
		case DIALOG_DELETE_BAD_PROJECT_ID: {
			return AlertDialogs.createAlert(this, getString(R.string.editor_delete_project), 0, getString(R.string.editor_load_error),
					getString(R.string.yes), new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int which) {
							ApiService.deleteProject(VideoEditorActivity.this, bundle.getString(PARAM_PROJECT_PATH));
		...
		case DIALOG_EDIT_PROJECT_NAME_ID: {
			...
			return AlertDialogs.createEditDialog(this, getString(R.string.editor_edit_project_name), mProject.getName(),
		...
		case DIALOG_EXPORT_OPTIONS_ID: {
			...
			return ExportOptionsDialog.create(this, new ExportOptionsDialog.ExportOptionsListener() {
				@Override
				public void onExportOptions(int movieHeight, int movieBitrate) {
					mPendingExportFilename = FileUtils.createMovieName(MediaProperties.FILE_MP4);
					ApiService.exportVideoEditor(VideoEditorActivity.this, mProjectPath, mPendingExportFilename, movieHeight, movieBitrate);
	 ...
	 * 
	 * @param target
	 *            one of the "play", "rewind", "next", and "prev" buttons in the control panel
	 */
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent extras) {
		...
		case REQUEST_CODE_CAPTURE_VIDEO: {
			if (mProject != null) {
				ApiService.addMediaItemVideoUri(this, mProjectPath, ApiService.generateId(), mInsertMediaItemAfterMediaItemId, mCaptureMediaUri,
						MediaItem.RENDERING_MODE_BLACK_BORDER, mProject.getTheme());
		...
		case REQUEST_CODE_CAPTURE_IMAGE: {
			if (mProject != null) {
				ApiService.addMediaItemImageUri(this, mProjectPath, ApiService.generateId(), mInsertMediaItemAfterMediaItemId, mCaptureMediaUri,
						MediaItem.RENDERING_MODE_BLACK_BORDER, MediaItemUtils.getDefaultImageDuration(), mProject.getTheme());
				mInsertMediaItemAfterMediaItemId = null;
			} else {
		...
		case REQUEST_CODE_IMPORT_VIDEO: {
			...
			if (mProject != null) {
				if ("media".equals(mediaUri.getAuthority())) {
					ApiService.addMediaItemVideoUri(this, mProjectPath, ApiService.generateId(), mInsertMediaItemAfterMediaItemId, mediaUri,
							MediaItem.RENDERING_MODE_BLACK_BORDER, mProject.getTheme());
				} else {
					// Notify the user that this item needs to be downloaded.
					Toast.makeText(this, getString(R.string.editor_video_load), Toast.LENGTH_LONG).show();
		...
		case REQUEST_CODE_IMPORT_IMAGE: {
			...
			if (mProject != null) {
				if ("media".equals(mediaUri.getAuthority())) {
					ApiService.addMediaItemImageUri(this, mProjectPath, ApiService.generateId(), mInsertMediaItemAfterMediaItemId, mediaUri,
							MediaItem.RENDERING_MODE_BLACK_BORDER, MediaItemUtils.getDefaultImageDuration(), mProject.getTheme());
				} else {
					// Notify the user that this item needs to be downloaded.
					Toast.makeText(this, getString(R.string.editor_image_load), Toast.LENGTH_LONG).show();
					// When the download is complete insert it into the project.
		...
		case REQUEST_CODE_IMPORT_MUSIC: {
			...
			if (mProject != null) {
				ApiService.addAudioTrack(this, mProjectPath, ApiService.generateId(), data, true);
			} else {
		...
		case REQUEST_CODE_EDIT_TRANSITION: {
			...
			final String afterMediaId = extras.getStringExtra(TransitionsActivity.PARAM_AFTER_MEDIA_ITEM_ID);
			final String transitionId = extras.getStringExtra(TransitionsActivity.PARAM_TRANSITION_ID);
			final long transitionDurationMs = extras.getLongExtra(TransitionsActivity.PARAM_TRANSITION_DURATION, 500);
			if (mProject != null) {
				mMediaLayout.editTransition(afterMediaId, transitionId, type, transitionDurationMs);
			} else {
		...
		case REQUEST_CODE_PICK_OVERLAY: {
			...
			final String mediaItemId = extras.getStringExtra(OverlayTitleEditor.PARAM_MEDIA_ITEM_ID);
			final String overlayId = extras.getStringExtra(OverlayTitleEditor.PARAM_OVERLAY_ID);
			final Bundle bundle = extras.getBundleExtra(OverlayTitleEditor.PARAM_OVERLAY_ATTRIBUTES);
			if (mProject != null) {
				final MovieMediaItem mediaItem = mProject.getMediaItem(mediaItemId);
				if (mediaItem != null) {
					if (overlayId == null) {
						ApiService.addOverlay(this, mProject.getPath(), mediaItemId, ApiService.generateId(), bundle,
								mediaItem.getAppBoundaryBeginTime(), OverlayLinearLayout.DEFAULT_TITLE_DURATION);
					} else {
						ApiService.setOverlayUserAttributes(this, mProject.getPath(), mediaItemId, overlayId, bundle);
					}
		...
		case REQUEST_CODE_KEN_BURNS: {
			final String mediaItemId = extras.getStringExtra(KenBurnsActivity.PARAM_MEDIA_ITEM_ID);
			final Rect startRect = extras.getParcelableExtra(KenBurnsActivity.PARAM_START_RECT);
			final Rect endRect = extras.getParcelableExtra(KenBurnsActivity.PARAM_END_RECT);
			if (mProject != null) {
				mMediaLayout.addEffect(EffectType.EFFECT_KEN_BURNS, mediaItemId, startRect, endRect);
	...
	protected boolean showPreviewFrame() {
		...
		if (mProject != null && !mPreviewThread.isPlaying() && mPendingExportFilename == null) {
			...
			mPreviewThread.previewFrame(mProject, mProject.getPlayheadPos(), mProject.getMediaItemCount() == 0);
		}
	...
	/**
	 * Convert the time to dimension At zoom level 1: one activity width = 1200 seconds At zoom level 2: one activity width = 600 seconds ... At zoom
	 * level 100: one activity width = 12 seconds
	 * 
	 ...
	 *            The zoom level
	 * @param updateControl
	 *            true to set the control position to match the zoom level
	 */
	...
	protected void setAspectRatio(final int aspectRatio) {
		final FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mSurfaceView.getLayoutParams();
	...
	protected void initializeFromProject(boolean updateUI) {
		...
		if (mAddMediaItemVideoUri != null) {
			ApiService.addMediaItemVideoUri(this, mProjectPath, ApiService.generateId(), mInsertMediaItemAfterMediaItemId, mAddMediaItemVideoUri,
					MediaItem.RENDERING_MODE_BLACK_BORDER, mProject.getTheme());
		...
		if (mAddMediaItemImageUri != null) {
			ApiService.addMediaItemImageUri(this, mProjectPath, ApiService.generateId(), mInsertMediaItemAfterMediaItemId, mAddMediaItemImageUri,
					MediaItem.RENDERING_MODE_BLACK_BORDER, MediaItemUtils.getDefaultImageDuration(), mProject.getTheme());
		...
		if (mAddAudioTrackUri != null) {
			ApiService.addAudioTrack(this, mProject.getPath(), ApiService.generateId(), mAddAudioTrackUri, true);
			mAddAudioTrackUri = null;
		}

		if (mAddTransitionAfterMediaId != null) {
			mMediaLayout.addTransition(mAddTransitionAfterMediaId, mAddTransitionType, mAddTransitionDurationMs);
			mAddTransitionAfterMediaId = null;
		}

		if (mEditTransitionId != null) {
			mMediaLayout.editTransition(mEditTransitionAfterMediaId, mEditTransitionId, mEditTransitionType, mEditTransitionDurationMs);
			mEditTransitionId = null;
			mEditTransitionAfterMediaId = null;
		}

		if (mAddOverlayMediaItemId != null) {
			ApiService.addOverlay(this, mProject.getPath(), mAddOverlayMediaItemId, ApiService.generateId(), mAddOverlayUserAttributes, 0,
		...
		if (mEditOverlayMediaItemId != null) {
			ApiService.setOverlayUserAttributes(this, mProject.getPath(), mEditOverlayMediaItemId, mEditOverlayId, mEditOverlayUserAttributes);
		...
		if (mAddEffectMediaItemId != null) {
			mMediaLayout.addEffect(mAddEffectType, mAddEffectMediaItemId, mAddKenBurnsStartRect, mAddKenBurnsEndRect);
	...
	private void showExportProgress() {
		...
		mExportProgressDialog.setButton(getString(android.R.string.cancel), new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				cancelExport();
			}
	...
	private void cancelExport() {
		ApiService.cancelExportVideoEditor(VideoEditorActivity.this, mProjectPath, mPendingExportFilename);
		mPendingExportFilename = null;
		mExportProgressDialog = null;
	}
	...
	private class PreviewThread extends Thread {
		 ...
		 *            The frame time
		 * @param clear
		 *            true to clear the output
		 */
		public void previewFrame(final VideoEditorProject project, final long timeMs, final boolean clear) {
			...
			mQueue.add(new Runnable() {
				...
				public void run() {
					if (clear) {
						...
						try {
							if (project.renderPreviewFrame(mSurfaceHolder, timeMs, overlayData) < 0) {
								logd("Cannot render preview frame at: " + timeMs + " of " + mProject.computeDuration());
		...
		public void renderMediaItemFrame(final MovieMediaItem mediaItem, final long timeMs) {
			...
			mQueue.add(new Runnable() {
				...
				public void run() {
					try {
						if (mProject.renderMediaItemFrame(mSurfaceHolder, mediaItem.getId(), timeMs) < 0) {
							logd("Cannot render media item frame at: " + timeMs + " of " + mediaItem.getDuration());
						}
					} catch (Exception ex) {
		...
		private void startPreviewPlayback(final VideoEditorProject project, final long fromMs) {
			...
			mQueue.add(new Runnable() {
				...
				public void run() {
					try {
						project.startPreview(mSurfaceHolder, fromMs, -1, false, 3, new VideoEditor.PreviewProgressListener() {
							@Override
							public void onStart(VideoEditor videoEditor) {
							}

							@Override
							public void onProgress(VideoEditor videoEditor, final long timeMs, final VideoEditor.OverlayData overlayData) {
								mMainHandler.post(new Runnable() {
									@Override
									public void run() {
										...
										if (mPreviewState == PREVIEW_STATE_STARTED || mPreviewState == PREVIEW_STATE_STOPPING) {
											movePlayhead(timeMs);
										}
									}
								});
							...
							@Override
							public void onError(VideoEditor arg0, int arg1) {
		...
		/**
		 * The preview started. This method is always invoked from the UI thread.
		 * 
		 * @param project
		 *            The project
		 */
		private void previewStarted(VideoEditorProject project) {
			...
			VideoEditorActivity.this.getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
		}
		...
		private void stopPreviewPlayback() {
			...
			case PREVIEW_STATE_STARTING: {
				logd("stopPreviewPlayback: State was PREVIEW_STATE_STARTING " + "now PREVIEW_STATE_STOPPING");
			...
			default: {
				throw new IllegalArgumentException("stopPreviewPlayback state: " + mPreviewState);
			}
			}
		}
		 ...
		 * 
		 * @param error
		 *            true if the preview stopped due to an error
		 */
		private void previewStopped(boolean error) {
			...
			VideoEditorActivity.this.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
		}
		...
		private boolean isPlaying() {
			return mPreviewState == PREVIEW_STATE_STARTING || mPreviewState == PREVIEW_STATE_STARTED;
		}
